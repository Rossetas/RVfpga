...
#define RGPIO_INTE_Buttons    0x8000180C
#define RGPIO_PTRIG_Buttons   0x80001810
#define RGPIO_CTRL_Buttons    0x80001818
#define RGPIO_INTS_Buttons    0x8000181C
/**************************************************************************************************/
...
unsigned int n=1;
unsigned int BTNC=0, Sws0=0;
unsigned int buttons_value=0;

void GPIO_ISR(void)
{
  buttons_value = M_PSP_READ_REGISTER_32(GPIO_Buttons);     // lectura de los pulsadores
  BTNC = buttons_value & (0x10);                            // lectura del pulsador central, aplicando mascara
  Sws0 = ((M_PSP_READ_REGISTER_32(GPIO_SWs)>>16) & (0x1));  // lectura del Switch[0]
  ...
  M_PSP_WRITE_REGISTER_32(RPTC_LRC, 0x2FAF080 / n); 
  ...
  M_PSP_WRITE_REGISTER_32(RGPIO_INTS_Buttons, 0x0);
...
void PTC_ISR(void)
{
  //M_PSP_WRITE_REGISTER_32(SegEn_ADDR, SegDisplCount);
...
void GPIO_Buttons_Initialization(void)
{
  /* Configure LEDs and Switches */
  M_PSP_WRITE_REGISTER_32(GPIO_INOUT_Buttons, 0x0);     /* GPIO_INOUT */
  //M_PSP_WRITE_REGISTER_32(GPIO_LEDs, 0x0);            /* GPIO_LEDs */

  /* Configure GPIO interrupts */
  M_PSP_WRITE_REGISTER_32(RGPIO_INTE_Buttons, 0x1F);       /* RGPIO_INTE */   // habilitar los 5 pulsadores como interrupcion
  M_PSP_WRITE_REGISTER_32(RGPIO_PTRIG_Buttons, 0x1F);      /* RGPIO_PTRIG */  // habilitar los 5 pulsadores para su uso
  M_PSP_WRITE_REGISTER_32(RGPIO_INTS_Buttons, 0x0);       /* RGPIO_INTS */
  M_PSP_WRITE_REGISTER_32(RGPIO_CTRL_Buttons, 0x1);       /* RGPIO_CTRL */
}

void PTC_Initialization(void)
{
  M_PSP_WRITE_REGISTER_32(RPTC_CTRL, 0x80);
  M_PSP_WRITE_REGISTER_32(RPTC_LRC, 0x02FAF080); 
  M_PSP_WRITE_REGISTER_32(RPTC_CTRL, (0x21)); 
}
