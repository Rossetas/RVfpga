...
// ----------------------------------------------
// Paso 4: Crear la rutina de servicio de interrupción:
// Debe  realiza el incremento del contador de la pantalla y se limpia la interrupción.
void PTC_ISR(void)
{
  // Write Display:
  M_PSP_WRITE_REGISTER_32(SegDig_ADDR, SegDisplCount);

  /* Clear GPIO interrupt MOD 0x00*/
  M_PSP_WRITE_REGISTER_32(RPTC_CNTR, 0x0);   //¿Hay que hacer clear a la interupcion de PTC? Apagar bandera de int (1 para borrar)   

  // Detener la interrupción externa:
...
// ----------------------------------------------
// Paso 3: Función que inicializa las interrupciones del PTC.
void Initialization_PTC(void)
{
  // Inicializar PTC:
  M_PSP_WRITE_REGISTER_32(RPTC_CNTR, 0x00);         // RPTC_CNTR(Contador)  : Iniciar en 0 (00)
  M_PSP_WRITE_REGISTER_32(RPTC_LRC,  0x2FAF080);    // RPTC_LRC(Referencia) : 50M  (0x2FAF080)
  M_PSP_WRITE_REGISTER_32(RPTC_HRC,  0x00);         // RPTC_HRC(Referencia) : (00)
  M_PSP_WRITE_REGISTER_32(RPTC_CTRL, 0x001100001);  // RPTC_CTRL(Control)   : (0x001100001)ADD int enable, (REPETIR DIRECCION PARA reset).
}
// ----------------------------------------------
...
int main(void)
{
  ...
  // ---------------------------------------------------------------
  // Inicializar IRQ3:
  // Paso 1: Agregar un segunda fuente de interrupción.
  ExternalIntLine_Initialization(3, 7, PTC_ISR);      /* Initialize line IRQ3,priority of 7. Set GPIO_ISR as the Interrupt Service Routine */
  // Paso 2: Poner en 1 irq_ptc_enable.
  M_PSP_WRITE_REGISTER_32(Select_INT, 0x10);          //IRQ3 interrupt (irq_ptc_enable)
