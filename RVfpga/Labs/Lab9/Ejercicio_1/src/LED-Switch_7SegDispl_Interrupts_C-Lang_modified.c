...
void GPIO_ISR(void)
{
  ...
  /**************************************************************************************************/
  /**************************************************************************************************/
  if (SegDisplCount >= 0xA){

    pspExtInterruptSetPriority(4, 10); // modificacion del umbral en tiempo de ejecucion
  }
  /**************************************************************************************************/
  /**************************************************************************************************/
...
void PTC_ISR(void)
{
  ...
  M_PSP_WRITE_REGISTER_32(RPTC_CTRL, (0x40)); // habilitar bandera de INT (interrupcion pendiente)
  M_PSP_WRITE_REGISTER_32(RPTC_CTRL, (0x21)); // habilitar contador e interrupcion
  M_PSP_WRITE_REGISTER_32(SegDig_ADDR, SegDisplCount);

    if (SegDisplCount == 0xF){
      pspExtInterruptsSetThreshold(8); // se modifica el umbral a nivel 8
    }
...
{
  M_PSP_WRITE_REGISTER_32(RPTC_LRC, 0x02FAF080); //frecuencia 50kk, se le asigna ese valor al LRC
  M_PSP_WRITE_REGISTER_32(RPTC_CTRL, (0x21)); // habilitar contador (bit 0) y habilitar la interrupcion (bit 5), practica 8
  // bit(0)   EN - Cuando se pone a 1, el RPTC_CNTR se incrementa.
  /* bit(5)   INTE - Cuando está a 1, el PTC activa una interrupción cuando el valor de RPTC_CNTR es igual al valor de RPTC_LRC o RPTC_HRC. 
              Cuando la señal está a 0, las interrupciones se enmascaran. */
}
/**************************************************************************************************/
/**************************************************************************************************/
...
int main(void)
{
  ...
  pspExtInterruptsSetThreshold(4); // modificacion inicial del umbral
  ...
  /**************************************************************************************************/
  /* step 2.2 */
  ExternalIntLine_Initialization(3, 5, PTC_ISR); // modificado
  M_PSP_WRITE_REGISTER_32(Select_INT, 0x3); // habilitar interrupciones lineas 3 y 4 (irq_ptc y irq_gpio)
  /**************************************************************************************************/
  /**************************************************************************************************/
...
// pspExtInterruptsSetThreshold(threshold)
// pspExtInterruptSetPriority(interrupt_source, priority)

/* void pspExtInterruptsSetThreshold(u32_t uiThreshold);
  Establece el umbral de prioridad de las interrupciones externas en el PIC
*/

/* void pspExtInterruptSetPriority(u32_t uiIntNum, u32_t uiPriority);
  Establece la prioridad de una línea de interrupción dada
*/
